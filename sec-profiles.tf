...
resource "panos_url_filtering_security_profile" "fw1_url_profile" {
  ...
  depends_on = [
    ...
    panos_custom_url_category.fw1_rone_external_url,
  ]
  ...
  block_categories = [
    "adult", "abortion", "command-and-control", "cryptocurrency", "extremism", "gambling",
    "hacking", "high-risk", "malware", "nudity", "phishing", "questionable", "ransomware",
    "sex-education", "rone-external-url"
  ]

  alert_categories = [
    "unknown", "medium-risk", "parked", "peer-to-peer", "proxy-avoidance-and-anonymizers",
    "real-time-detection", "shareware-and-freeware"
  ]

  allow_categories = [
    "rone-msp-allow", "rone-msp-block"
  ]
...
# Duplicate same resources as above but change provider = panos.fw2 and resource names to fw2_*

# (Let me know if you want me to send the full fw2 block too, identical to fw1 above with only provider/resource name changes)
